...
using DedsiAuthorization.DedsiUsers.Queries;
...
public class AuthorizationController(
    IContractDedsiUserQuery contractDedsiUserQuery,
    IOpenIddictApplicationManager applicationManager) : AbpController
{
    ...
    private async Task<IActionResult> ExchangePassword(OpenIddictRequest request, Object? application)
    {
        ...
        var dedsiUser = await contractDedsiUserQuery.FindDedsiUserByAccountAsync(request.Username, request.Password, HttpContext.RequestAborted);
        if (dedsiUser  is null)
        {
